	...
	$.widget("thomaskahn.smoothDivScroll", {
		...
		_create: function () {
			...
			if (o.touchScrolling && el.data("enabled")) {
				...
				el.data("scrollWrapper").kinetic({
					...
					//filterTarget: function(target, e){
						...
						//	return !(/area|a|input|img/i.test(target.tagName));
						//}
					//}
				});
			}
			...
			*****************************************/
			...
			var _calcScrollXPos = function(e, $this, isLeft){
				if (o.hotSpotScrolling) {
					var x = (e.originalEvent.touches ? e.originalEvent.touches[0].pageX : e.pageX) - $this.offset().left;
					
					if( isLeft ) x = el.data("hotSpotWidth") - x;
			...
			var _startScrolling = function(e, $this, isLeft){
				if( !el.data("scrollXPos") && e && $this ) _calcScrollXPos(e, $this, isLeft);
				
				_doScrolling(isLeft);
			};
			var _doScrolling = function(isLeft){
				if (el.data("scrollXPos") > 0 && el.data("enabled")) {
					el.data("scrollWrapper").scrollLeft(el.data("scrollWrapper").scrollLeft() + ((isLeft ? -1 : 1) * el.data("scrollXPos") * el.data("speedBooster")));
			...
			var _endScrolling = function(isLeft){
				if (o.hotSpotScrolling) {
					clearInterval(el.data(isLeft ? "leftScrollingInterval" : "rightScrollingInterval"));
					el.data("scrollXPos", 0);
			...
			var scrollingEvents = o.isMobile ? o.hotSpotScrollingEvents.mobile : o.hotSpotScrollingEvents.pc;
			
			// Mouseover right hotspot - scrolling
			el.data("scrollingHotSpotRight").bind(scrollingEvents.start, function (e) {
				if (o.hotSpotScrolling) {
					// Stop any ongoing animations
					el.data("scrollWrapper").stop(true, false);
			...
			el.data("scrollingHotSpotRight").bind(scrollingEvents.move, function (e) {
				_calcScrollXPos(e, $(this));
			});
			
			// Mouseout right hotspot - stop scrolling
			el.data("scrollingHotSpotRight").bind(scrollingEvents.end, function () {
				_endScrolling();
			});
			...
			*****************************************/
			...
			el.data("scrollingHotSpotLeft").bind(scrollingEvents.start, function (e) {
				if (o.hotSpotScrolling) {
					// Stop any ongoing animations
					el.data("scrollWrapper").stop(true, false);
			...
			el.data("scrollingHotSpotLeft").bind(scrollingEvents.move, function (e) {
				_calcScrollXPos(e, $(this), true);
			});
			
			// mouseout left hotspot
			el.data("scrollingHotSpotLeft").bind(scrollingEvents.end, function () {
				_endScrolling(true);
			});
			...
			*****************************************/
			...
			AUTOSCROLLING
			*****************************************/
			// The $(el).imagesLoaded event handler is used because the width of the elements are not calculated
		...
		**********************************************************/
		recalculateScrollableArea: function () {
			...
			el.data("scrollableArea").children(o.countOnlyClass).each(function () {
				var $this = $(this).clone().appendTo("body").css({position:"absolute", visibility:"hidden", display:""});
				...
				tempScrollableAreaWidth = tempScrollableAreaWidth + $this.get(0).getBoundingClientRect().width;
